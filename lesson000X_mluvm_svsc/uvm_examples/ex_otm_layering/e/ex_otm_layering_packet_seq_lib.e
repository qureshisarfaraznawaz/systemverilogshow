...
extend LAYERED ex_otm_layering_packet_sequence {
    upper_layer_id: uint;
...
extend ex_otm_layering_packet_sequence_kind: [LAYERED_LONG];
...
extend ex_otm_layering_packet_sequence_kind: [LAYERED_SHORT];

extend LAYERED_SHORT ex_otm_layering_packet_sequence {
    upper_layer_id: uint;
    is_relevant(): bool is only {
        result = (upper_layer_id == driver.stream_id);
    };
